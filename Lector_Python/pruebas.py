...
def main() -> None:
    ...
    last_alert_level = None  # 'moderado', 'alto', 'critico' o None
    current_hour_alerts = []  # Almacena los niveles de alerta en la hora actual
    ...
    while True:
        ...
        if valores_doub:
            ...
            # Check thresholds
            umbral_minimo = 15
            print(f"The difference is: {diferencia:.2f}")
            
            # Guardar en base de datos si:
            # 1. Diferencia es menor al umbral mínimo (15) O
            # 2. Diferencia es mayor a 120
            if diferencia <= umbral_minimo or diferencia >= 120:
                Lector.send_data_to_table(datetime.now(), min_num, max_num)
            
            # Lógica de alertas solo para diferencias >= 120
            if diferencia >= 120:
                # Determinar nivel de alerta actual
                current_level = None
                if diferencia >= 375:
                ...
                # Añadir a las alertas de esta hora
                current_hour_alerts.append(current_level)
                
                # Verificar si es una nueva hora (para resetear el contador)
                current_hour = datetime.now(Lector.ZONE_ID).hour
                if hasattr(Lector, 'last_hour_checked') and current_hour != Lector.last_hour_checked:
                    # Hora cambiada, resetear contadores
                    current_hour_alerts = []
                Lector.last_hour_checked = current_hour
                
                # Lógica de envío de SMS
                if current_hour_alerts:
                    predominant_level = max(set(current_hour_alerts), key=current_hour_alerts.count)
                    
                    # Si no había alerta previa o subió de nivel
                    if last_alert_level is None or (
                        (predominant_level == 'critico' and last_alert_level != 'critico') or
                        (predominant_level == 'alto' and last_alert_level in [None, 'moderado']) or
                        (predominant_level == 'moderado' and last_alert_level is None)
                    ):
                        # Enviar SMS solo si es un nuevo nivel más alto
                        if predominant_level != last_alert_level:
                            if predominant_level == 'moderado':
                                mensaje = f"Alerta Moderada: Diferencia de {diferencia:.2f}. Valores entre {min_num:.2f} y {max_num:.2f}"
                            elif predominant_level == 'alto':
                                mensaje = f"Alerta Alta: Diferencia de {diferencia:.2f}. Valores entre {min_num:.2f} y {max_num:.2f}"
                            elif predominant_level == 'critico':
                                mensaje = f"ALERTA CRÍTICA: Diferencia de {diferencia:.2f}. Valores entre {min_num:.2f} y {max_num:.2f}"
                            
                            Lector.send_message(mensaje)
                            last_alert_level = predominant_level
            else:
                # No hay alerta actual (diferencia < 120)
                if last_alert_level is not None:
                    # Esperar a la siguiente hora para notificar bajada de nivel
                    pass
